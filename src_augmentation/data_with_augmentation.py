    ...
    def __getitem__(self, idx):
        if 'preprocess' in self.config and self.config['preprocess']:
            ...
            if self.train and self.config['rot_aug']:
                ...
                # aug1: dropout agents with prob 0.1
                a_range = len(data['gt_preds'])
                selected_agents = give_idx(a_range)

                for key in ['city', 'orig', 'gt_preds', 'has_preds', 'selected_nodes', 'dis_matrix']:
                    if key in data:
                        if key == 'gt_preds' or key=='has_preds':
                            new_data[key] = ref_copy(data[key][selected_agents])
                        else:
                            new_data[key] = ref_copy(data[key])

                #dt = np.random.rand() * self.config['rot_size']#np.pi * 2.0
                dt = random.uniform(-np.pi/2, np.pi/2)
                theta = data['theta'] + dt
                ...
                new_data['feats'] = data['feats'][selected_agents].copy()
                new_data['feats'][:, :, :2] = np.matmul(new_data['feats'][:, :, :2], rot)
                new_data['ctrs'] = np.matmul(data['ctrs'][selected_agents], rot)
              
                # aug3: add random noise
                new_data['ctrs'][1:] += np.random.normal(0, 0.1, [new_data['ctrs'].shape[0]-1, 2])
                new_data['feats'][:, 1:, :2] += np.random.normal(0, 0.1, [new_data['feats'].shape[0], 19, 2])
                ...
                new_data = dict()
                if self.train:
                    for key in ['city', 'orig', 'gt_preds', 'has_preds', 'theta', 'rot', 'feats', 'ctrs', 'graph', 'selected_nodes', 'dis_matrix']:
                        if key in data:
                            new_data[key] = ref_copy(data[key])
                    data = new_data
                else:
