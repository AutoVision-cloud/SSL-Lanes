...
config["horovod"] = True
config["opt"] = "adam"
config["num_epochs"] = 40 #36 #Change1: Set epochs to 40
config["lr"] = [1e-3, 1e-4]
config["lr_epochs"] = [32]
...
config["num_scales"] = 1 #6 #Change5: set num_scales to 1
config["n_actor"] = 128
config["n_map"] = 128
...
config["cls_coef"] = 1.0
config["reg_coef"] = 1.0
config["mgn"] = 0.2 #Change2: change margin to 0.5
config["cls_th"] = 2.0
...
class Net(nn.Module):
    ...
    def __init__(self, config):
        ...
        #self.a2m = A2M(config) #Change3: just do sequential fusion
        #self.m2m = M2M(config)
        self.m2a = M2A(config)
        self.a2a = A2A(config)
    ...
    def forward(self, data: Dict) -> Dict[str, List[Tensor]]:
        ...
        #nodes = self.a2m(nodes, graph, actors, actor_idcs, actor_ctrs) #Change4: Sequential fusion
...
class MapNet(nn.Module):
    ...
    def __init__(self, config):
        ...
        for i in range(2): #4): #Change6: Change blk nos from 4 to 2
            for key in fuse:
                if key in ["norm"]:
