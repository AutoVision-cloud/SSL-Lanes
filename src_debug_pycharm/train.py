...
# Open Intellij, go to menu bar "Help" -> "Show log in file manager" and then
# delete ALL files and directorys in there. I never had a freeze anymore.
...
# Data loader for training
    ...
    train_sampler = DistributedSampler(dataset, num_replicas=1, rank=0) # hvd.size(), rank=hvd.rank())
    ...
    # Data loader for evaluation
    dataset = Dataset(config["val_split"], config, train=False)
    val_sampler = DistributedSampler(dataset, num_replicas=1, rank=0) # hvd.size(), rank=hvd.rank())
    val_loader = DataLoader(
        dataset,
        batch_size=config["val_batch_size"],
        num_workers=config["val_workers"],
...
def worker_init_fn(pid):
    np_seed = 123 # hvd.rank() * 1024 + int(pid)
...
def train(epoch, config, train_loader, net, loss, post_process, opt, val_loader=None):
    ...
    display_iters = int(
        config["display_iters"] / (1 * config["batch_size"]) # (hvd.size() * config["batch_size"])
    )
    val_iters = int(config["val_iters"] / (1 * config["batch_size"])) # (hvd.size() * config["batch_size"]))
